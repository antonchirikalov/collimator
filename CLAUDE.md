...
- `collimator/emit_workflow.py` — узлы графа → скрипт воркфлоу, по файлу на сегмент между
  чекпойнтами;
- `library/` — данные, не код: 27 агентов, типы артефактов, шаблоны конвейеров;
...
Три вещи держат петлю от холостого хода:
...
- **Агенты оставляют черновые файлы в корне репозитория.** После прогона там нашлись
  `temp_data.py`, `rounds_output.json`, `final_structured_output.json` — и попали в коммит,
  потому что `git add -A`. `tools/sweep_junk.py` их не заметил: он ищет пустые файлы, а эти с
  содержимым. Перед коммитом смотреть `git status` глазами, а не только `--short`.
