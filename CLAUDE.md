...
Сгенерированный скрипт воркфлоу прогоняется `dry_run.mjs` **до** живого запуска, и не в двух
режимах, а во всех, какие у него есть ветки. Для `explainer-article` их четыре: `ok` со
`config.fresh` (полный путь), `ok` без него (продолжение с диска), `bad` (все ветви провалов) и
`config.correctors:false` (конвейер без корректоров). Он подменяет `agent()` заглушкой, отвечающей по схеме вызова, и исполняет настоящий
...
- `collimator/emit_workflow.py` — узлы графа → скрипт воркфлоу, по файлу на сегмент между
  чекпойнтами;
- `library/` — данные, не код: 28 агентов, типы артефактов, шаблоны конвейеров;
...
## Три записи, ни одна из которых не пересказ

Полный ответ на «всё ли правильно передалось и ничего ли не изменилось» собирается из трёх
независимых источников, и ни один не является воспоминанием модели:

- **`handoff.md`** — каждый вызов по порядку: метка, агент, модель, входные порты с путями,
  выходной файл. Составляет скрипт из того, что и так знает;
- **`tools.jsonl`** — что нашла каждая детерминированная проверка. Пишет инструмент, который мерил;
- **ревизия** — вычитание прочитанного из лежащего на диске, сироты поимённо.

**Агента-логгера здесь нет намеренно.** Агент — наименее надёжный элемент конвейера: один молча
урезал вывод инструмента с пяти кругов до двух и стоил полутора миллионов токенов, другой вернул
абсолютные пути там, где скрипт передал относительные. Доверять запись тому, что эта запись и
проверяет, — значит терять её ровно тогда, когда она нужна. Скрипт всё это уже знает; спрашивать
не у кого.
...
- **Два прогона на одном каталоге портят состояние.** Убитый по отчёту прогон продолжал
  ...
  правил второй круг и по чьим замечаниям, не установить. Перед запуском убеждаться, что по
  каталогу не идёт другой прогон.
