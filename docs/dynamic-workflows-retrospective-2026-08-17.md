...
4. Как врёт агент-посредник. Обнаруживается по странным числам в отчёте.
...
## Словарь: три слова, которые дальше нужны
...
аналитик, писатель, два корректора, два критика, носильщик проверок, копировальщик, писец записей
и составитель брифа.
...
Предыдущая версия проекта всё это имела, и всё это было убито намеренно: платформа делает
исполнение лучше и меняется быстрее, чем мы успевали бы за ней.
...
файлов на 260. Семьдесят процентов найденного — сироты.
...
Скрипт не умеет читать каталог. Значит размерность любого веера — сколько параллельных агентов
запускать и как назвать их файлы — обязана быть известна заранее. Заранее известны только аспекты
...
скрипт знать не может. Спросить у агента, куда он положил, нельзя, и это отдельный инвариант: путь
придумывает скрипт, и поле «путь» в схеме ответа создаёт канал, где «сообщить путь» выглядит как
«записать файл».
...
Не обещанием, а вычитанием.

Скрипт копит каждый путь, доехавший до агента, и копит его тем единственным действием, которым
путь и может быть потреблён — попаданием в текст задачи. Обойти учёт, передав файл агенту,
невозможно: это одна и та же строчка кода.

В конце этапа каталог прогона перечисляется целиком, и одно вычитается из другого:
...
Сироты называются поимённо, а не считаются: число говорит, что что-то пропало, имя говорит что.

Правило ревизии — «прочитано агентом или объявлено записью». Снимок черновика читает человек, а не
агент, поэтому он объявляется явной строкой в коде. Это единственное исключение, и оно названо на
месте: иначе запись, существующая для человека, каждый прогон считалась бы потерей.

## 4. Как врёт агент-посредник

Скрипт не может ни читать файлы, ни запускать команды. Значит любое измерение приезжает к нему
через агента-носильщика: тот выполняет команду, читает вывод инструмента и возвращает его по
...
Поэтому петля останавливается ещё и по полке: два круга подряд не улучшили лучший достигнутый
счёт — работа окончена, остальное решает автор по отчёту о незакрытом. Круги 9–11 одного прогона
...
Пишет инструмент, а не агент: носильщику создавать файлы запрещено, и этот запрет уже дважды себя
...
каждый может вернуть не то. В последнем прогоне из 43 агентов больше двадцати были носильщиками.
...
весь источник собственных ошибок. Из десяти инвариантов того движка после переезда выжили два, и
оба относятся не к исполнению, а к контракту между скриптом и агентом.
...
номера кругов, элементы веера. Раздел ввода-вывода в инструкции агента генерируется из контракта и
