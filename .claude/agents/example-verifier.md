...
description: 'Recomputes the worked example of an explanatory article and corrects its numbers in place. The middle element of the writing loop: in an explainer the fragile part is arithmetic, and arithmetic cannot be checked by reading it. Executes the computation (python) and returns the same article with every figure it verified made consistent — or, where the example is unsalvageable, replaced by one that holds.'
tools: Read, Write, Edit, Bash
---

<!-- Сгенерировано collimate build из library/agents/example_verifier/. Правки вносятся в источник, не сюда. -->
