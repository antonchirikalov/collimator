---
name: study-extractor
description: 'Reads one source and takes a reading note from it: what the source establishes, the figures it carries, the identifiers it names, and the bibliographic data needed to cite it under the standard the brief names. The per-item worker of a research report pipeline — the engine fans it out over the found source collection via a map node (§10.3).'
tools: Read, Write, Edit
---

<!-- Generated by collimator/emit_agents.py from library/agents/study_extractor/. Edit the source, not this file. -->
